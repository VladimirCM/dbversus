...
async function categoriasInsertarScylladb(total: number): Promise<number> {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    //agrego usando $fetch
    await $fetch("/api/scylladb/categoria", {
      method: "POST",
      headers: {
...
async function categoriasConsultarScylladb(): Promise<number> {
  let start = new Date().getTime()
  await $fetch("/api/scylladb/categorias", {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
    },
...
async function categoriasConsultarAzarScylladb(total: number): Promise<number> {
  let start = new Date().getTime()
  for (let i = 1; i <= total; i++) {
    ...
    await $fetch("/api/scylladb/categoria/" + randomUuid, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
...
async function categoriasActualizarScylladb(total: number): Promise<number> {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    //agrego usando $fetch
    await $fetch("/api/scylladb/categoria/" + uuid, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
...
async function categoriasEliminarScylladb(total: number): Promise<number> {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    await $fetch("/api/scylladb/categoria/" + id, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
